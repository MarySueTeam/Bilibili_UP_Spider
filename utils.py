...
class Bili_UP:
    ...
    def get_video_list(self):
        ...
        self.log.info(f"[bold green]GET {len(self.video_list)} VIDEOS FOR {self.user_name}[/]")
        self.insert_db()

    def insert_db(self):
        cursor = self.db.cursor()
        for item in self.video_list:
            sql = """insert into video_info(aid,bvid,title,author,description,cover_img,is_download) values ('%s','%s','%s','%s','%s','%s','%s');""" % (item['aid'], item['bvid'], item['title'], item['author'], item['description'],
                item['pic'], '0')
            try:
                cursor.execute(sql)
                self.db.commit()
                self.log.debug(f"[bold green]INSERT {item['bvid']} SUCCESSFULLY[/]")
    ...
    def downloader(self):
        ...
        for row in data:
            ...
            _sql = """update video_info SET is_download=1 where bvid='%s';""" % (row[2])
            try:
                cursor.execute(_sql)
                self.db.commit()
            except:
    ...
    def check_download(self, all_videos):
        ...
        cursor.execute(sql)
        data = cursor.fetchall()
        self.log.info(f"[bold green blink]{data[0][0]}/{all_videos} VIDEOS DOWNLOADED[/]")
